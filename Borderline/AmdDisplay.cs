...
internal static class AmdDisplay
{
    ...
    public static string? TryApply(int top, int bottom, int left, int right, out string? message)
    {
        ...
        var current = 0; var defaultVal = 0; var min = 0; var max = 0; var step = 0;
        ...
        if (_adapter < 0) { _adapter = adapter; _display = display; _originalUnderscan = current; }
        ...
        message = $"AMD underscan {target}% via driver.";
        return null;
    }

    public static bool Restore()
    {
        if (_adapter < 0) return false;
        try { EnsureInit(); } catch { return false; }
    ...
    {
        adapter = 0; display = 0; error = "no AMD adapter found";
        var numAdapters = 0;
        if (ADL_Adapter_NumberOfAdapters_Get(ref numAdapters) != ADL_OK || numAdapters <= 0) return false;

        var infoSize = Marshal.SizeOf<ADLDisplayInfo>();
        for (adapter = 0; adapter < numAdapters; adapter++)
        {
            var active = 0;
            if (ADL_Adapter_Active_Get(adapter, ref active) != ADL_OK || active == 0) continue;

            var numDisplays = 0;
            if (ADL_Display_DisplayInfo_Get(adapter, ref numDisplays, IntPtr.Zero, 0) != ADL_OK || numDisplays <= 0) continue;

            var buffer = Marshal.AllocHGlobal(numDisplays * infoSize);
            try
            {
                if (ADL_Display_DisplayInfo_Get(adapter, ref numDisplays, buffer, 1) != ADL_OK) continue;
                for (var i = 0; i < numDisplays; i++)
                {
                    var info = Marshal.PtrToStructure<ADLDisplayInfo>(buffer + (i * infoSize));
                    display = info.displayID.iDisplayLogicalIndex;
                    var cur = 0; var def = 0; var min = 0; var max = 0; var step = 0;
                    if (ADL_Display_Underscan_Get(adapter, display, ref cur, ref def, ref min, ref max, ref step) == ADL_OK)
                    {
                        error = string.Empty;
                        return true;
                    }
